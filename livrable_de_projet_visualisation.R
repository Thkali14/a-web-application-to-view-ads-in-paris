...
ui <- fluidPage(
  fluidRow(
    ...
    titlePanel(HTML("<h1><center><font size=14> Paris declaration in between 2018-2019</font></center></h1>")) ,   
  ),
  fluidRow(
    column(5,
           sidebarPanel(
             selectInput('Type',"Declaration type",choices = dansmarue %>% distinct(TYPE)),
             selectInput('p_code',"Postal Code", choices = dansmarue %>% distinct(CODE_POSTAL)),
